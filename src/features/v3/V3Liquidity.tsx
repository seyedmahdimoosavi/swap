...
export default function V3Liquidity() {
  ...
  const [t1Sel, setT1Sel] = useState(tokens[1]?.address ?? tokens[0]?.address ?? '');
  ...
  {
    const dp = sorted ? currentPriceRef.current : currentPriceRef.current > 0 ? 1 / currentPriceRef.current : 0;
    if (minP > 0 && maxP > minP && dp > 0) summaryEfficiency = Math.max(1, (dp * 2) / (maxP - minP)).toFixed(1) + 'x';
  }
  ...
  const drawChart = useCallback(() => {
    ...
    data.forEach((d, i) => {
      ...
      if ((ctx as any).roundRect) (ctx as any).roundRect(x, y, Math.max(barWidth - 1, 1), barH, [2, 2, 0, 0]);
  ...
  const updateLiquidity = useCallback(async () => {
    ...
    try {
      ...
      if (poolAddr && poolAddr !== ethers.constants.AddressZero) {
        ...
        currentPriceRef.current = v3SqrtPriceToPrice(slot0.sqrtPriceX96, info0.decimals, info1.decimals);
        const dp = isSorted ? currentPriceRef.current : 1 / currentPriceRef.current;
        setPriceDisplay(`1 ${i0.symbol} = ${dp.toFixed(6)} ${i1.symbol}`);
        setMinPrice((prev) => (parseFloat(prev) === 0 ? (dp * 0.8).toFixed(6) : prev));
        setMaxPrice((prev) => (parseFloat(prev) === 0 ? (dp * 1.2).toFixed(6) : prev));
      } else {
    ...
    if (userAddress && provider) {
      try {
        ...
        const [b0, b1] = await Promise.all([c0.balanceOf(userAddress), c1.balanceOf(userAddress)]);
        setBal0('Balance: ' + parseFloat(ethers.utils.formatUnits(b0, i0.decimals)).toFixed(4));
        setBal1('Balance: ' + parseFloat(ethers.utils.formatUnits(b1, i1.decimals)).toFixed(4));
      } catch {
        /* ignore */
      }
    }
  ...
  async function addNFTToWallet(tokenId: string) {
    ...
    try {
      await eth.request({
        ...
        params: { type: 'ERC721', options: { address: V3_POSITION_MANAGER, tokenId: tokenId.toString() } },
      });
  ...
  async function addLiquidity() {
    ...
    const amount0Desired = dep0 > 0 ? v3SafeParseUnits(dep0, info0.decimals) : ethers.BigNumber.from(0);
    const amount1Desired = dep1 > 0 ? v3SafeParseUnits(dep1, info1.decimals) : ethers.BigNumber.from(0);
    ...
    if (tickLower >= tickUpper) {
      showStatus('Invalid tick range. Please adjust your price range.', 'error');
      return;
    }

    setAdding(true);
    setMintedTokenId(null);
    setTx({ html: '<span class="v3-spinner"></span> Preparing...', type: 'pending' });
    try {
      const posManager = new ethers.Contract(V3_POSITION_MANAGER, V3_POSITION_MANAGER_ABI, signer);

      if (amount0Desired.gt(0)) {
        const chk = await v3CheckBalance(reads, userAddress, token0, amount0Desired);
        if (!chk.sufficient) {
      ...
      if (amount1Desired.gt(0)) {
        const chk = await v3CheckBalance(reads, userAddress, token1, amount1Desired);
        if (!chk.sufficient) {
          setTx({ html: `❌ ${chk.error}`, type: 'error' });
          return;
        }
      ...
      if (!poolAddrRef.current) {
        setTx({ html: '<span class="v3-spinner"></span> Creating pool...', type: 'pending' });
        const initPrice = priceLower * priceUpper > 0 ? Math.sqrt(priceLower * priceUpper) : 1;
        const sqrtPriceX96 = v3ComputeSqrtPriceX96(initPrice);
        if (sqrtPriceX96.lte(ethers.BigNumber.from(MIN_SQRT_RATIO)) || sqrtPriceX96.gte(ethers.BigNumber.from(MAX_SQRT_RATIO))) {
          setTx({ html: '❌ Computed initial price is out of range. Please adjust the price range.', type: 'error' });
          return;
        }
        const createTx = await posManager.createAndInitializePoolIfNecessary(token0, token1, feeTier, sqrtPriceX96);
        setTx({ html: `<span class="v3-spinner"></span> Creating pool...<br>${txLink(createTx.hash)}`, type: 'pending' });
        await createTx.wait();
        const factoryAddr = await getV3Factory(reads);
        if (factoryAddr) {
          const factory = new ethers.Contract(factoryAddr, V3_FACTORY_ABI_EX, reads);
          poolAddrRef.current = await factory.getPool(token0, token1, feeTier);
        }
      }

      setTx({ html: '<span class="v3-spinner"></span> Approving tokens...', type: 'pending' });
      if (amount0Desired.gt(0)) {
        const c0 = new ethers.Contract(token0, ERC20_ABI, signer);
        const ap0 = amount0Desired.add(amount0Desired.div(100).gt(1000) ? amount0Desired.div(100) : ethers.BigNumber.from(1000));
        ...
        if (al0.lt(amount0Desired)) {
          if (!al0.isZero()) {
            const r = await c0.approve(V3_POSITION_MANAGER, 0, { gasLimit: 100000 });
            await r.wait();
          }
          const a = await c0.approve(V3_POSITION_MANAGER, safe0, { gasLimit: 100000 });
          await a.wait();
        }
      }
      if (amount1Desired.gt(0)) {
        const c1 = new ethers.Contract(token1, ERC20_ABI, signer);
        const ap1 = amount1Desired.add(amount1Desired.div(100).gt(1000) ? amount1Desired.div(100) : ethers.BigNumber.from(1000));
        ...
        if (al1.lt(amount1Desired)) {
          if (!al1.isZero()) {
            const r = await c1.approve(V3_POSITION_MANAGER, 0, { gasLimit: 100000 });
            await r.wait();
          }
          const a = await c1.approve(V3_POSITION_MANAGER, safe1, { gasLimit: 100000 });
          await a.wait();
        }
      }

      setTx({ html: '<span class="v3-spinner"></span> Minting position NFT...', type: 'pending' });
      ...
      try {
        ...
        const msg = v3GetErrorMessage(gasErr, 'Mint would fail. Please check token amounts and price range.');
        setTx({
      ...
      const mintTx = await posManager.mint(mintParams, { gasLimit: gasEstimate.mul(120).div(100) });
      setTx({ html: `<span class="v3-spinner"></span> Transaction sent! Waiting for confirmation...<br>${txLink(mintTx.hash)}`, type: 'pending' });
      ...
      for (const log of receipt.logs) {
        try {
          ...
          if (parsed.name === 'Transfer' && parsed.args.to && parsed.args.to.toLowerCase() === userAddress.toLowerCase()) {
      ...
      setTx({
        html: `✅ Liquidity position minted successfully!<br>${txLink(mintTx.hash)}${
          idStr ? `<br><span style="font-size:0.75rem; color:var(--gold-dark);">NFT Token ID: #${idStr}</span>` : ''
        }`,
        type: 'success',
      });
      ...
      setTx({ html: `❌ ${v3GetErrorMessage(e, 'Failed to add liquidity')}`, type: 'error' });
    } finally {
      setAdding(false);
    }
  }
    ...
    <div className="card">
      ...
      <div className="v3-pair-row">
        <div className="v3-pair-select" style={{ flex: 1 }}>
          <select className="token-select" style={{ width: '100%' }} value={t0Sel} onChange={(e) => setT0Sel(e.target.value)}>
            {tokens.map((t) => (
        ...
        <div className="v3-pair-select" style={{ flex: 1 }}>
          <select className="token-select" style={{ width: '100%' }} value={t1Sel} onChange={(e) => setT1Sel(e.target.value)}>
            {tokens.map((t) => (
              <option key={t.address} value={t.address}>
                {t.symbol}
              </option>
      ...
      <div className="fee-tier-section">
        ...
        <div className="fee-tiers">
          {FEE_TIERS.map((f) => (
            <button key={f.fee} className={`fee-tier-btn${feeTier === f.fee ? ' active' : ''}`} onClick={() => setFeeTier(f.fee)}>
      ...
      <div className="price-range-section">
        ...
        <div className="price-range-inputs">
          <div className="price-range-box">
            ...
            <div className="price-range-input-wrap">
              <button className="price-range-btn" onClick={() => adjustPrice('min', -1)}>−</button>
              <input type="number" className="price-range-value" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} />
              <button className="price-range-btn" onClick={() => adjustPrice('min', 1)}>+</button>
            </div>
          ...
          <div className="price-range-box">
            ...
            <div className="price-range-input-wrap">
              <button className="price-range-btn" onClick={() => adjustPrice('max', -1)}>−</button>
              <input type="number" className="price-range-value" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} />
              <button className="price-range-btn" onClick={() => adjustPrice('max', 1)}>+</button>
            </div>
            <div className="price-range-per">{`${t1Info.symbol} per ${t0Info.symbol}`}</div>
          </div>
        ...
        <div className="price-range-presets">
          {[10, 20, 50, 100].map((p) => (
            <button key={p} className={`range-preset-btn${activePreset === p ? ' active' : ''}`} onClick={() => setRangePreset(p)}>
              {p === 100 ? 'Full Range' : `±${p}%`}
            </button>
          ))}
        </div>
      ...
      <div className="input-group">
        ...
        <div className="input-wrapper">
          <input type="number" className="token-input" placeholder="0.0" value={deposit0} onChange={(e) => setDeposit0(e.target.value)} />
        </div>
      </div>
      <div className="input-group">
        <div className="input-label">
        ...
        <div className="input-wrapper">
          <input type="number" className="token-input" placeholder="0.0" value={deposit1} onChange={(e) => setDeposit1(e.target.value)} />
        </div>
      </div>
      ...
      {isConnected ? (
        <button className="action-btn" style={{ width: '100%' }} onClick={addLiquidity} disabled={adding}>
      ...
      {mintedTokenId && (
        <button className="v3-add-nft-btn" onClick={() => addNFTToWallet(mintedTokenId)}>
          <span className="nft-icon">🖼️</span> Add NFT #{mintedTokenId} to Wallet
        </button>
      )}
    </div>
  );
