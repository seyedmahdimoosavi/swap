...
export default function Pools() {
  const { provider, readOnlyProvider, factoryContract, readOnlyFactoryContract } = useWeb3();
    ...
    try {
      ...
      for (let i = 0; i < totalPairs; i++) {
        try {
          ...
          const pair = new ethers.Contract(pairAddress, PAIR_ABI, currentProvider);
  ...
  return (
    <div id="poolsSection">
      <div className="card">
        ...
        {loading && (
          <div className="py-6 text-center text-[0.85rem] text-white/55">{loadingText}</div>
        )}
        ...
        {!loading && pools.length === 0 && (
          <div className="py-6 text-center text-[0.85rem] text-white/55">No active pools found</div>
        )}
      </div>
