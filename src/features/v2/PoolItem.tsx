...
function FormField({
  label, balance, onMax, value, onChange,
}: {
  label: string; balance: string; onMax: () => void; value: string; onChange: (v: string) => void;
}) {
  return (
    <div className="mb-3">
      <div className="mb-2 flex items-center justify-between text-[0.78rem]">
        <span className="text-white/55">{label}</span>
        <span className="cursor-pointer text-white/55 hover:text-white" onClick={onMax}>{balance}</span>
      </div>
      <input
        type="number" placeholder="0" step="any" min="0" value={value}
...
export default function PoolItem({ pool, onRefresh }: { pool: Pool; onRefresh: () => void }) {
  ...
  const [rawLpBalance, setRawLpBalance] = useState<ethers.BigNumber | null>(null);
  ...
  const toggleAdd = async () => {
    ...
    try {
      ...
      setRawBalance0(b0);
      setRawBalance1(b1);
      setBalance0(`Balance: ${parseFloat(ethers.utils.formatUnits(b0, t0.decimals)).toFixed(4)}`);
      setBalance1(`Balance: ${parseFloat(ethers.utils.formatUnits(b1, t1.decimals)).toFixed(4)}`);
    } catch (err) {
  ...
  const toggleWithdraw = async () => {
    ...
    try {
      ...
      setRawLpBalance(bal);
      setLpBalanceText(`LP Balance: ${parseFloat(ethers.utils.formatUnits(bal, 18)).toFixed(6)}`);
    } catch (err) {
  ...
  const calculatePoolAmounts = async (changed: 0 | 1, a0: string, a1: string) => {
    try {
      ...
      if (reserves.reserve0.gt(0) && reserves.reserve1.gt(0)) {
        const r0 = parseFloat(ethers.utils.formatUnits(reserves.reserve0, t0.decimals));
        const r1 = parseFloat(ethers.utils.formatUnits(reserves.reserve1, t1.decimals));
        const ratio = r1 / r0;
  ...
  const submitAdd = async () => {
    ...
    try {
      ...
      await tx.wait();
      setAddStatus({ message: 'Liquidity added successfully!', type: 'success' });
      setAmount0('');
      setAmount1('');
  ...
  const calculateWithdrawAmounts = async (lp: string) => {
    ...
    try {
      ...
      setReceive0(parseFloat(ethers.utils.formatUnits(a0, t0.decimals)).toFixed(6));
      setReceive1(parseFloat(ethers.utils.formatUnits(a1, t1.decimals)).toFixed(6));
      setShowReceive(true);
    } catch (err) {
  ...
  const submitWithdraw = async () => {
    ...
    try {
      ...
      const lpBalance: ethers.BigNumber = await pairContract.balanceOf(userAddress);
      ...
      const allowance = await pairContract.allowance(userAddress, ROUTER_ADDRESS);
      if (allowance.lt(lpWei)) {
        if (!allowance.isZero()) {
          setWStatus({ message: 'Resetting LP allowance...', type: 'info' });
          await (await pairContract.approve(ROUTER_ADDRESS, 0, { gasLimit: 100000 })).wait();
        }
        setWStatus({ message: 'Approving LP tokens...', type: 'info' });
        await (
          await pairContract.approve(ROUTER_ADDRESS, ethers.constants.MaxUint256, { gasLimit: 100000 })
        ).wait();
      }
      ...
      setWStatus({ message: 'Liquidity withdrawn successfully!', type: 'success' });
      setLpAmount('');
      setShowReceive(false);
      setTimeout(onRefresh, 2000);
    } catch (error) {
  ...
  const addLPToWallet = async () => {
    ...
    try {
      ...
      const wasAdded = await window.ethereum.request({
        ...
        params: { type: 'ERC20', options: { address: pool.address, symbol: 'DOT1-LP-V2', decimals: 18 } },
      });
      if (wasAdded) alert('LP token added to wallet successfully!');
    } catch (error) {
      alert('Failed to add LP token: ' + ((error as Error).message || 'Unknown error'));
    }
  };

  const addDisabled = !(parseFloat(amount0) > 0 && parseFloat(amount1) > 0) || addSubmitting;
    ...
    <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`rounded-lg px-4 py-2 text-[0.82rem] font-semibold transition-colors ${
                open === 'add' ? 'bg-[#161616] text-white' : 'bg-gold text-black hover:bg-gold-light'
              }`}
            >
              + Add liquidity
            </button>
            <button
              ...
              className={`rounded-lg px-4 py-2 text-[0.82rem] font-semibold transition-colors ${
                open === 'withdraw' ? 'bg-[#161616] text-white' : 'bg-gold text-black hover:bg-gold-light'
              }`}
            >
              {'\u2212 Withdraw'}
            </button>
      ...
      {open === 'add' && (
        <div className="rounded-2xl bg-[#2a2a2a] p-5">
          ...
          <FormField label={`${t0.symbol} Amount`} balance={balance0} onMax={() => setMax(0)} value={amount0} onChange={onAmount0} />
          <FormField label={`${t1.symbol} Amount`} balance={balance1} onMax={() => setMax(1)} value={amount1} onChange={onAmount1} />
          {addStatus && <div className={`mb-3 text-[0.8rem] ${statusClass(addStatus.type)}`}>{addStatus.message}</div>}
          <button
            onClick={submitAdd}
          ...
          >
            {addSubmitting ? 'Processing...' : parseFloat(amount0) > 0 && parseFloat(amount1) > 0 ? 'Add liquidity' : 'Enter amounts'}
          </button>
        </div>
      )}
        ...
        <div className="rounded-2xl bg-[#2a2a2a] p-5">
          ...
          <div className="mb-3">
            <div className="mb-2 flex items-center justify-between text-[0.78rem]">
              ...
              <span className="cursor-pointer text-white/55 hover:text-white" onClick={setMaxLp}>
                {lpBalanceText}
              </span>
            </div>
            <input
          ...
          {showReceive && (
            <div className="mb-3 rounded-lg bg-[#1f1f1f] p-4">
              <div className="mb-2 text-[0.78rem] text-white/55">You will receive:</div>
          ...
          {wStatus && <div className={`mb-3 text-[0.8rem] ${statusClass(wStatus.type)}`}>{wStatus.message}</div>}
          <button
            onClick={submitWithdraw}
            disabled={wDisabled}
            className="w-full rounded-lg bg-gold py-3 text-[0.9rem] font-bold text-black transition-colors hover:bg-gold-light disabled:opacity-45"
          >
            {wSubmitting ? 'Processing...' : parseFloat(lpAmount) > 0 ? 'Withdraw' : 'Enter amount'}
          </button>
        </div>
      )}
    </div>
...
async function approve(
  ...
  if (allowance.lt(desired)) {
    ...
    await (await token.approve(ROUTER_ADDRESS, desired, { gasLimit: 100000 })).wait();
  }
}
